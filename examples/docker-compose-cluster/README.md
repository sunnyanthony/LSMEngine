...
nonzero raw lag in an empty database. In leader mode `LSM_GATEWAY_READ_READY_MIN` must be 0 or 1;
more than one readable leader cannot be required. Override that smoke gate with `LSM_GATEWAY_READ_READY_MIN` and
`LSM_GATEWAY_READ_READY_MAX_LAG` when testing slower environments; set
`LSM_GATEWAY_READ_READY_MAX_LAG=-1` to disable only the smoke read-ready gate.
...
Use `LSM_GATEWAY_READY_MIN_REACHABLE`, `LSM_GATEWAY_READY_MIN_READ_READY`,
and `LSM_GATEWAY_READY_MAX_READ_APPLY_LAG` to configure the container's `/readyz`.
The smoke checks container readiness after its first committed write catches up;
an empty cluster may not yet meet a strict raw apply-lag bound.
