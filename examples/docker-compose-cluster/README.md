...
`LSM_GATEWAY_READY_MAX_READ_APPLY_LAG` when tuning the Compose `/readyz`
healthcheck, and `LSM_GATEWAY_READ_READY_MIN` /
`LSM_GATEWAY_READ_READY_MAX_LAG` when tuning the script's explicit
`wait-gateway` gate; set `LSM_GATEWAY_READ_READY_MAX_LAG=-1` to disable only the
explicit smoke read-ready wait. The smoke then checks accepted write-status
lookup through the same gateway endpoint. It also exercises range scans and
