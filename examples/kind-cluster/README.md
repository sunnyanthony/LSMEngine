...
sequence with `wait-cluster --min-applied-index --require-compatible` before reading followers, so it
...
apply the committed write/delete sequence with `wait-cluster --min-applied-index --require-compatible`
