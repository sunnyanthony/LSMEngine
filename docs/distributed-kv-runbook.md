...
- `Stats.WAL.SegmentCount`, `Stats.WAL.TotalBytes`, and related
  ...
  The checkpoint is the last successfully persisted contiguous prefix, not the
  highest completed SSTable sequence. Lag is a sequence distance (sequences may
...
`--endpoint-dns-name`, `--endpoint-dns-service`, `--endpoint-dns-proto`, and
`--endpoint-dns-scheme`.
...
not raft ReadIndex, lease-read, or a complete linearizable read protocol.
Discovery and the data read are separate requests: leadership may change between
them, and the read does not revalidate leadership with a quorum. Leader read
failures and not-found results do not fall back to follower data.
Accepted write status lookups keep best-effort endpoint fallback because the
request-status tracker is node-local.
...
Missing or null apply-lag observations are rejected when the gate is enabled.
...
nodes, leader read mode, and a write leader. After the first write and final
delete are applied, it checks the selected leader against the read-lag bound.
Empty-cluster bootstrap/election entries must not block the first write.
Override that smoke gate with
...
the selected leader within the configured apply-lag bound after the first write
and delete, not before traffic on an empty cluster. Follower catch-up is verified with
