...
- M1 raft peer ingress/transport foundation: when `raft.peers` has multiple members, etcd-raft can bootstrap static peer ids, route outbound peer messages through `CommitLogOptions.Transport` (`CommitLogPeerTransport`), and accept inbound peer messages via `HandlePeerMessages`; the public boundary uses LSM-owned `CommitLogPeerMessage` envelopes while the built-in provider keeps etcd raft protobuf details internal. Network service wiring, quorum-backed commits, durable raft log storage, and membership lifecycle remain deferred.
