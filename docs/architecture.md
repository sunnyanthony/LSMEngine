...
- M1 shard replica membership foundation: add/remove replica operations update shard control metadata through the committed control path, persist to `control_state.json`, reject removing the current leader, and replicate to followers. This does not yet reconfigure etcd-raft voters or bootstrap a new node automatically.
...
- `<data>/raft/commitlog-<node-id>/segments/segment-<first-index>.json`: builtin etcd-raft provider log entry segments.
  ...
  - This is a segmented durability and raft-log snapshot restore foundation. Provider-owned raft log compaction can be enabled with `CommitLogOptions.SnapshotPolicy`; builtin raft snapshot data is captured from the engine after local apply. Live follower reset/catch-up and membership policy are still planned.
