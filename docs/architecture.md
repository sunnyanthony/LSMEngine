...
- M1 distributed surface: fixed shard metadata + manual control operations (leader transfer/split/rebalance/drain/add-replica/remove-replica) exposed through server APIs.
...
- M1 raft peer ingress/transport foundation: when `raft.peers` has multiple members, etcd-raft can bootstrap static peer ids, route outbound peer messages through `CommitLogOptions.Transport` (`CommitLogPeerTransport`), and accept inbound peer messages via `HandlePeerMessages`. Server mode can wire HTTP peer delivery with `raft.peer_urls`, posting LSM-owned `CommitLogPeerMessage` envelopes to `/cluster/raft/messages`; `raft.join_peer_urls` pre-registers HTTP endpoints for future joiners without adding them to initial raft voters. HTTP peer delivery resolves endpoints through the LSM-owned `RaftPeerResolver` contract, while route-aware client writes and `lsmctl` operator commands resolve node HTTP endpoints through `NodeEndpointResolver`. The current implementations are static config and reloaded endpoint files, while DNS/Kubernetes/other discovery providers can fit behind the same layers. etcd raftpb encoding stays inside the builtin provider adapter. The builtin provider owns a background tick/advance loop so elections and committed-entry delivery continue without local proposals, and the engine closes that loop before closing WAL/table handles. The engine also exposes manual `AddRaftPeer` / `RemoveRaftPeer` hooks backed by builtin etcd-raft ConfChange proposals, and server mode exposes them through `/cluster/nodes/{id}/raft-add` and `/cluster/nodes/{id}/raft-remove`. New nodes can start with `raft.join: true` to skip local raft bootstrap and wait for the existing cluster's ConfChange/log/snapshot stream. Incoming raft snapshots with LSM payload data can restore an empty engine or logically reset a lagging non-empty follower by applying snapshot entries and tombstoning locally visible keys absent from the snapshot. Automatic service discovery and orchestration remain deferred.
- M1 raft storage foundation: the builtin etcd-raft provider persists raft hard state, raft snapshots, and segmented log entry files under `<data>/raft/commitlog-<node-id>/`, restores runtime position after restart, and continues committed indexes instead of re-bootstraping from an empty in-memory raft log. `CommitLogOptions.SnapshotPolicy` can enable provider-owned raft log snapshot/compaction after a configured number of applied entries while retaining a recent tail for peer catch-up. When the builtin provider is wired through the engine, raft snapshot data is an LSM-owned state-machine payload captured only after the matching commit index has applied locally. Legacy single-file `commitlog-<node-id>.json` state is still accepted for migration.
- M1 LSM state snapshot payload foundation: the engine can encode visible KV state plus control-plane state into an LSM-owned state-machine snapshot payload and restore it into an empty engine, persisting restored data through WAL/control-state files. The builtin raft provider stores that payload in raft snapshot data after engine apply and applies incoming raft snapshot data through the same engine-owned path. For non-empty lagging followers, snapshot reset is logical rather than file-level: snapshot-visible keys are written, local keys missing from the snapshot are tombstoned, and older snapshots are ignored.
...
- M1 shard replica membership foundation: add/remove replica operations update shard control metadata through the committed control path, persist to `control_state.json`, reject removing the current leader, and replicate to followers. Raft voter membership is a separate provider-level operation (`AddRaftPeer` / `RemoveRaftPeer`) so shard metadata cannot accidentally assume node networking/bootstrap is ready.
- M1 3-node smoke coverage: integration tests now exercise a three-node etcd-raft write through in-process peer delivery, HTTP peer delivery, manual dynamic join, static real multi-process `lsmctl serve` nodes, leader restart with remaining-quorum writes, rolling restart with per-step quorum writes, minority partition write rejection, follower long-outage catch-up after multiple quorum writes, and real multi-process dynamic join with joined-node restart, proving a leader write can commit and materialize on followers in the foundation path.
...
- M1 commit-log runtime observability: `ClusterStatus` includes `commit_log_runtime` (`mode/index/term/snapshot_index/applied_index/apply_lag/leader/replicas/write_available/leader_known/health/last_error_*`) so control-plane health, local write availability, leader knowledge, local apply progress, and recent commit-path failures can be inspected without parsing logs. Multi-peer transport scaffolding reports `mode=raft_transport_foundation`, not real multi-node raft operation.
...
- M1 routing metadata/retry surface: server exposes route table snapshots (`/cluster/routes`) and write errors include retryable route hints (`revision/shard/leader`); `server.Gateway` uses cached routes, route-hint updates, refresh fallback, and configurable bounded write attempts/backoff for route-aware client writes. `GET /gateway/status` exposes process-local routing counters for write attempts, retries, failures, read attempts, read fallback attempts, read failures, route refreshes, route refresh failures, route-hint updates, and per-backend read/write/status-probe attempts, successes, and failures; `GET /gateway/metrics` exposes the gateway readiness/backend/routing subset in text metrics form for scraping. `lsmctl gateway` can expose those route-aware writes behind one HTTP endpoint and proxies reads through best-effort endpoint fallback with configurable healthy-endpoint balancing, failure-aware adaptive ordering, and cooldown for recently failed endpoints. Gateway backend discovery can be driven by config-backed endpoint files or DNS SRV settings, with CLI overrides for one-off processes.
- M1 CDC foundation: server exposes node-local retained per-shard change events via `/cdc/events` with `offset/limit` and retention/restart signaling (`start_offset` and `dropped_before`), plus `/cdc/status` for machine-readable source/durability metadata and retained windows. The current contract is intentionally not WAL-backed durable CDC: events are emitted from applied writes, retained in memory per node, and lost on restart or retention overflow. After WAL replay or state-snapshot restore, the node raises `start_offset` to the materialized sequence so consumers can distinguish an empty recent feed from an unavailable pre-restart/pre-restore history. Durable CDC replay from WAL/raft log is deferred until the replication/snapshot model is stable.
...
- External dependency boundaries: third-party core libraries must sit behind an LSM-owned layer before they reach public/server APIs. `internal/lsm/iofs` is the IO example. The builtin etcd-raft integration follows the same rule: raftpb messages and storage details stay inside `internal/lsm/commitlog`, while public/server callers use `CommitLogConsensus`, `CommitLogOptions`, `CommitLogPeerMessage`, and LSM-owned snapshot contracts. If a dependency becomes hard to replace without public API churn, add or tighten an LSM-owned adapter before adding more call sites. See `docs/dependency-boundaries.md`.
- Runtime progress distinguishes raw log index/lag from the optional LSM-owned state-machine committed index/lag. The raft adapter excludes configuration/no-op entries from the latter and reconstructs it from snapshot plus committed-entry replay. Engine apply computes the distance without advancing its durable replay floor. Unknown provider boundaries remain absent, not zero.
- Membership snapshots use an LSM-owned boundary contract: the provider supplies the committed log boundary and preceding mutation index, waits for every local proposer apply acknowledgement, and the engine verifies its applied mutation index before capture. Configuration-only progress can extend the encoded snapshot boundary without changing the live engine replay floor. Existing snapshots are refreshed after membership changes even below the normal snapshot interval; an old snapshot is never relabeled with future membership. Legacy snapshotters without this boundary capability retain their exact-index contract.
- On restart, the persisted engine snapshot is restored before committed-tail replay. Independently newer WAL/control prefixes are preserved; equal-sequence data is materialized again to finish a potentially interrupted snapshot restore. Startup raft replay must not replace a persisted engine payload with a metadata-only snapshot before the engine attaches.
- HTTP peer delivery supports the optional LSM-owned `CommitLogReportingPeerTransport` contract. Per-target resolution/HTTP outcomes are queued back to the builtin provider; only the raft adapter maps them to unreachable/snapshot reports. Failed sends do not block already committed-entry application or Ready advancement. Snapshot reports are matched by leader term, snapshot index, and send generation so old results cannot cancel a new attempt. Delivery success is not quorum commitment. Legacy `Send`-only transports cannot report asynchronous failures; production custom transports should implement the optional reporting contract. Transport concurrency/backpressure and a full network fault matrix remain separate hardening work.
- The provider background loop is the sole Raft tick source. Client commit/election/membership waits may drain Ready and wait for peer input, but must not accelerate logical time at their polling rate or restart an in-progress campaign. Membership waits follow their context deadline, not a fixed count of request polls.
...
- The commit-log/raft path follows the same rule. Etcd-raft is an implementation detail of the built-in `etcd-raft` provider; engine code depends on the commit-log contract (`CommitControl`, `CommitData`, committed entries, runtime status, `CommitLogPeerTransport`, and `CommitLogPeerMessage` envelopes) rather than directly on etcd raft state-machine details.
- Where a foundation branch temporarily exposes a library type at a boundary, the follow-up hardening task is to wrap it in an LSM-owned type before expanding the surface area. For raft peer-message ingress/transport, that wrapper is `CommitLogPeerMessage`; the remaining production-readiness gaps are network service wiring, durable raft storage, membership lifecycle, and quorum-backed data/control commits.
- See `docs/dependency-boundaries.md` for the rule, checklist, and current boundary status.
...
- CDC streams are not reconstructed during recovery in the current phase. Startup sets CDC `start_offset` after WAL replay, and state-snapshot restore/reset sets it to the snapshot sequence. Consumers must treat `/cdc/events` as a recent node-local feed, use `start_offset` / `oldest_offset` / `dropped_before` to detect gaps, and fall back to application-level resync when a gap or restart occurs.
...
- `<data>/raft/commitlog-<node-id>/segments/segment-<first-index>.json`: builtin etcd-raft provider log entry segments.
  ...
  - This is a segmented durability and raft-log snapshot restore foundation. Provider-owned raft log compaction can be enabled with `CommitLogOptions.SnapshotPolicy`; builtin raft snapshot data is captured from the engine after local apply and incoming snapshot data can restore an empty engine or logically reset a lagging follower. Manual raft ConfChange hooks and join-peer URL wiring exist, while automatic join/bootstrap policy is still planned.

## Configuration knobs
- Memtable: `MemtableKind` (`map`, `skiplist`, `sharded-skiplist`), `MemtableConcurrency`, `MemtableShards`, `MemtableArenaBlockSize`.
- WAL: `WALBlockSize`, `WALMaxSegmentBytes`, `WALRetainArchivedSegments`, `WALMaxRecord`, `WALAsync`, `WALQueueDepth`, `WALBatchMax`.
...
- Monitoring: `Stats()`, `/stats`, and `/metrics` expose memtable pressure, immutable flush backlog, WAL segment/byte/checkpoint-lag counters, SSTable count/bytes by level, L0 threshold pressure, configured compaction/backpressure/WAL thresholds, periodic compaction check interval, write-backpressure state/reject counts, compaction runtime activity counters, point-read probe counters, SSTable read-pipeline counters, and lifecycle health. WAL stats are local storage observations for the current node, not cluster-wide durability status. `CompactionPending` is an L0 threshold signal only; it is not a full compaction debt scheduler. Compaction runtime counters report trigger/run/step/error activity for the current process, not durable job history. Point-read probe counters are cumulative process-local observability for read amplification, not latency histograms or a full query profiler.
