...
- M1 commit-log runtime observability: `ClusterStatus` includes `commit_log_runtime` (`mode/index/term/leader/replicas/write_available/leader_known/health/last_error_*`) so control-plane health, local write availability, leader knowledge, and recent commit-path failures can be inspected without parsing logs. Multi-peer transport scaffolding reports `mode=raft_transport_foundation`, not real multi-node raft operation.
