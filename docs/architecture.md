...
- Distributed transport and membership: outbound HTTP peer transport, inbound raft message hooks, follower committed-entry apply, shard replica add/remove metadata, manual raft voter add/remove hooks, segmented raft log persistence, raft snapshot persistence/apply plumbing, provider-owned raft log snapshot policy, LSM state snapshot payload export/empty-restore, three-node smoke coverage, leader hints, bounded gateway retries, and retryable commit-log availability errors are present, but full LSM state-machine snapshot transfer/catch-up, automatic membership lifecycle, node bootstrap/join, and higher-level service discovery/load balancing remain deferred for later phases.
