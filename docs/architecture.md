...
- Distributed transport and membership: outbound HTTP peer transport, inbound raft message hooks, follower committed-entry apply, segmented raft log persistence, raft snapshot persistence/apply plumbing, three-node smoke coverage, leader hints, bounded gateway retries, and retryable commit-log availability errors are present, but state-machine snapshot generation, membership lifecycle, and higher-level service discovery/load balancing remain deferred for later phases.
...
- `<data>/raft/commitlog-<node-id>/segments/segment-<first-index>.json`: builtin etcd-raft provider log entry segments.
  ...
  - This is a segmented durability and snapshot restore foundation; automatic state-machine snapshot generation and membership catch-up policy are still planned.
