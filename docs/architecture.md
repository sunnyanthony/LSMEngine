...
- `<data>/raft/commitlog-<node-id>/segments/segment-<first-index>.json`: builtin etcd-raft provider log entry segments.
  ...
  - This is a segmented durability and raft-log snapshot restore foundation. Provider-owned raft log compaction can be enabled with `CommitLogOptions.SnapshotPolicy`; full LSM state-machine snapshot transfer and membership catch-up policy are still planned.
