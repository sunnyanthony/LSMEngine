...
- M1 3-node smoke coverage: integration tests now exercise a three-node etcd-raft write through in-process peer delivery, HTTP peer delivery, and real multi-process `lsmctl serve` nodes, proving a leader write can commit and materialize on followers in the foundation path.
