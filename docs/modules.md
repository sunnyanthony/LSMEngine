...
- `pkg/lsm/engine/cdc.go`: per-shard retained change stream buffer plus read/status APIs (`ReadCDCEvents`, `CDCStatus`); node-local and in-memory only, not WAL-backed durable CDC.
