...
- `pkg/lsm/engine/commitlog.go`: control/data commit-log adapter and provider selection.
  ...
  - server transport: `pkg/lsm/server/raft_transport.go` provides an HTTP adapter that posts `CommitLogPeerMessage` envelopes to peer server endpoints; YAML `raft.peer_urls` maps configured node names to URLs and is converted through `lsm.RaftPeerID`.
