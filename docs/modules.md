...
- `pkg/lsm/engine/commitlog.go`: control/data commit-log adapter and provider selection.
  ...
  - dependency boundary: public peer transport/ingress uses LSM-owned `CommitLogPeerMessage` envelopes. The builtin etcd-raft adapter encodes/decodes raftpb messages internally, so server/engine callers do not depend on etcd raft protocol structs.
