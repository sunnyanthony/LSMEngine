...
- `pkg/lsm/engine/commitlog.go`: control/data commit-log adapter and provider selection.
  ...
  - server transport: `pkg/lsm/server/raft_transport.go` provides an async HTTP adapter that posts `CommitLogPeerMessage` envelopes to peer server endpoints; YAML `raft.peer_urls` maps initial raft peer names to URLs, while `raft.join_peer_urls` adds transport endpoints for future joiners without making them bootstrap voters. Both maps are converted through `lsm.RaftPeerID`.
