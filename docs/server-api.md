...
- Control-plane persistence config:
  ...
  - `raft.peer_url_file` (optional): absolute path to a YAML/JSON node-name to server URL map. Server-mode HTTP peer delivery reloads this file through the LSM-owned `RaftPeerResolver` contract, with `raft.peer_urls` and `raft.join_peer_urls` as fallback endpoints. `lsmctl` cluster/operator commands load the same file from `--config`, then apply static config fallbacks and explicit CLI overrides. This supports operator-managed endpoint updates for future joiners without restarting existing nodes; automatic service discovery and membership repair remain later orchestration layers.
