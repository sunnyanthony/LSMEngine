...
- `lsmctl replacement-plan --new-node node-c ...` polls `/cluster/status`, selects exactly one unavailable old-node candidate when `--old-node` is omitted, reuses replacement preflight, and prints suggested `replace-node --dry-run` plus apply commands without submitting membership mutations.
- `lsmctl replacement-apply --new-node node-c ...` runs the same single-candidate planning step and then executes the replacement sequence once. This is an explicit operator/supervisor command, not a background repair loop.
