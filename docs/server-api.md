...
- Control mutations are executed through a commit-log adapter (`commitlog.provider`).
  ...
  - Static multi-peer bootstrap currently requires an outbound peer transport adapter (`CommitLogPeerTransport`, engine-side injection), and inbound peer-message handling is available via `HandlePeerMessages`; the boundary uses LSM-owned `CommitLogPeerMessage` envelopes while built-in etcd raft encoding stays internal. The builtin provider persists raft hard state/log entries under `<data>/raft/`, but network service routing, production raft WAL/snapshots, quorum-backed commits, and membership lifecycle are deferred.
