...
- `lsmctl cdc-events --addr <url> --shard <id> --offset <n> --limit <n>` prints `/cdc/events`; `--data-dir` opens a local engine for the same retained/gap inspection. Text output includes `start_offset` and `dropped_before` so operators can detect restart/restore or retention gaps.
