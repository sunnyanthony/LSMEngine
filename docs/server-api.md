...
- `GET /gateway/status` and `lsmctl gateway-status` include process-local routing counters for gateway write attempts, retries, failures, route refreshes, route refresh failures, and route-hint updates. These counters are diagnostic signals for the current gateway process, not durable metrics.
