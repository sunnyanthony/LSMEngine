...
- `lsmctl gateway --listen 127.0.0.1:8090 --bootstrap-url http://node-a:8080 --node-endpoint node-a=http://...` starts a thin HTTP gateway. The gateway exposes `/kv/put` and `/kv/delete` as route-aware writes through `server.Gateway`, `/kv/get` and `/kv/range` as best-effort endpoint fallback reads, and `/gateway/status` as the gateway's aggregated backend-node status view. It is a single client-facing endpoint for the current static distributed KV foundation, not a linearizable read layer.
