...
- `--require-compatible` additionally requires counted ready nodes to report identical, known LSM-owned compatibility versions; it does not validate migration or full rolling-upgrade safety.
...
- Control-plane persistence config:
  ...
  - Control-state version validation also applies to control payloads embedded in Raft state snapshots. Unsupported versions are rejected before local state changes; this guard does not establish full mixed-version cluster compatibility.
  - `/cluster/status` reports LSM-owned compatibility versions for operator comparison.
