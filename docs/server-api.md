...
- `lsmctl wait-cluster --node-endpoint node-a=http://... --timeout 60s` polls `/cluster/status` until the required number of healthy nodes is present and, by default, a write-available raft leader exists. `--min-ready` supports degraded-quorum waits, `--write-leader=false` disables the leader requirement, `--min-applied-index <n>` requires each counted node to have materialized at least that commit-log index, and `--max-apply-lag <n>` optionally gates on the raw runtime `index - applied_index` gap. Use `--min-applied-index <write-seq>` to wait for nodes to catch up to a specific committed write; the raw apply-lag gap can include provider/internal raft entries and is not a precise pending-KV-mutation count.
