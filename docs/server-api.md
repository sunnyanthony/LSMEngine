...
- Gateway read config:
  ...
  - `gateway_read_balance_policy` maps to `lsmctl gateway --read-balance-policy` and controls healthy endpoint ordering in `any` mode. `round_robin` is the default; `ordered` keeps sorted endpoint order while preserving failure fallback; `freshest` probes backend status and prefers lower reported apply lag for KV reads.
