...
- Control mutations are executed through a commit-log adapter (`commitlog.provider`).
  ...
  - Follower committed-entry apply exists as a foundation: committed entries received without a local pending proposal are applied to local control/data state after the commit-log provider reports them. Three-node smoke tests cover in-process delivery, HTTP peer delivery, a dynamic join path using `raft.join_peer_urls`, a static real multi-process `lsmctl serve` cluster, and a real multi-process dynamic join workflow that restarts the joined node, but automatic membership lifecycle and higher-level service discovery/load balancing remain deferred.
