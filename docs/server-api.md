...
- Control mutations are executed through a commit-log adapter (`commitlog.provider`).
  ...
  - Follower committed-entry apply exists as a foundation: committed entries received without a local pending proposal are applied to local control/data state after the commit-log provider reports them. Full multi-node runtime validation is still deferred.
