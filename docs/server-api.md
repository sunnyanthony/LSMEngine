...
- `lsmctl gateway --listen 127.0.0.1:8090 --bootstrap-url http://node-a:8080 --node-endpoint node-a=http://...` starts a thin HTTP gateway. The gateway exposes `/kv/put` and `/kv/delete` as route-aware writes through `server.Gateway`, `/kv/get` and `/kv/range` as best-effort endpoint fallback reads with healthy-endpoint rotation, `/kv/write-status/{request_id}` as best-effort request-status lookup across backends, `/healthz` for gateway process liveness, `/readyz` for backend write-leader readiness, and `/gateway/status` as the gateway's aggregated backend-node status view. Recent transport failures and 5xx responses are deferred behind healthy endpoints for a short cooldown, and `/gateway/status` reports per-backend `degraded` / `degraded_until` fields for that policy. It is a single client-facing endpoint for the current static distributed KV foundation, not a linearizable read layer.
