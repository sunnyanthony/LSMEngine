...
- Control mutations are executed through a commit-log adapter (`commitlog.provider`).
  ...
  - Follower committed-entry apply exists as a foundation: committed entries received without a local pending proposal are applied to local control/data state after the commit-log provider reports them. Three-node smoke tests cover in-process and HTTP peer delivery, but production raft WAL/snapshots, membership lifecycle, and higher-level service discovery/load balancing remain deferred.
