...
- `lsmctl replace-node --old-node node-a --new-node node-c ...` composes the manual replacement sequence for static clusters: raft-add the replacement, add it to shards that contain the old node, drain the old node, remove the old shard replicas, and raft-remove the old node. The replacement node must already be running and reachable; `examples/docker-compose-cluster/replace-node-smoke.sh` exercises this with join-mode node-d.
