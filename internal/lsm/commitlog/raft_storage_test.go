...
func TestRaftPersistentStorageTreatsEmptyHardStateFileAsLoaded(t *testing.T) {
	dataDir := t.TempDir()
	nodeID := uint64(3)
	statePath := filepath.Join(dataDir, "raft", "commitlog-0000000000000003", "hard_state.json")
	if err := os.MkdirAll(filepath.Dir(statePath), 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	data, err := json.Marshal(raftHardStateDisk{})
	if err != nil {
		t.Fatalf("marshal hard state: %v", err)
	}
	if err := os.WriteFile(statePath, data, 0o644); err != nil {
		t.Fatalf("write hard state: %v", err)
	}
	...
	if !loaded {
		t.Fatalf("expected existing hard state file to count as loaded")
	}
	if len(storage.entries) != 0 {
		t.Fatalf("expected no restored entries, got %d", len(storage.entries))
	}
	hardState, _, err := storage.InitialState()
	if err != nil {
		t.Fatalf("initial state: %v", err)
	}
	if hardState != (raftpb.HardState{}) {
		t.Fatalf("unexpected hard state: %+v", hardState)
	}
}
...
func TestRaftPersistentStorageRemovesStaleSegmentsOnOverwrite(t *testing.T) {
	...
	segmentsDir := filepath.Join(dataDir, "raft", "commitlog-0000000000000004", "segments")
