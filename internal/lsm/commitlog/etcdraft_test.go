...
func TestEtcdRaftConsensusJoinModeRequiresExistingPeer(t *testing.T) {
	_, err := newEtcdRaftConsensus(Config{
		Provider: ProviderEtcdRaft,
		DataDir:  t.TempDir(),
		NodeID:   "node-a",
	...
	if err == nil {
		t.Fatalf("expected join peer requirement error")
	}
	if !strings.Contains(err.Error(), "join requires") {
		t.Fatalf("expected join requirement error, got %v", err)
	}
}
...
func TestEtcdRaftConsensusKnownRemoteLeaderRejectsLocalCommit(t *testing.T) {
	...
	messages, err := encodeRaftPeerMessages([]raftpb.Message{
		{Type: raftpb.MsgHeartbeat, From: remoteLeader, To: consensus.nodeID, Term: 2, Commit: 3},
	})
	if err != nil {
...
func TestEtcdRaftConsensusChangeMembershipRejectsKnownRemoteLeader(t *testing.T) {
	transport := &recordingRaftTransport{}
	consensus, err := newEtcdRaftConsensus(Config{
		Provider:  ProviderEtcdRaft,
		DataDir:   t.TempDir(),
	...
	err = consensus.ChangeMembership(context.Background(), MembershipChange{
		Type:   MembershipChangeAddNode,
		NodeID: "node-d",
	})
	if !errors.Is(err, ErrNotLeader) {
	...
	status := consensus.RuntimeStatus()
	if status.LastErrorCode != "not_leader" {
		t.Fatalf("expected not leader diagnostic, got %+v", status)
	}
}
