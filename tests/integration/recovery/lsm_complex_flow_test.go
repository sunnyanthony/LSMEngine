//go:build test

package integration_test

import (
	"bytes"
	"testing"

	"lsmengine/pkg/lsm"
	"lsmengine/tests/integration/helpers"
)

func TestLSMComplexFlushCompactionRestartSnapshot(t *testing.T) {
	dir := t.TempDir()
	store, err := lsm.New(lsm.Options{
		DataDir:               dir,
		MemtableLimit:         4,
		WALSync:               false,
		CompactionL0Threshold: 2,
	})
	if err != nil {
		t.Fatalf("new lsm: %v", err)
	}

	waiter := helpers.StartCompactionWait(t)
	if err := store.Put([]byte("a"), []byte("1")); err != nil {
		t.Fatalf("put a: %v", err)
	}
	if err := store.Put([]byte("b"), []byte("2")); err != nil {
		t.Fatalf("put b: %v", err)
	}
	helpers.WaitForSSTableFiles(t, dir, 1)

	if err := store.Put([]byte("a"), []byte("3")); err != nil {
		t.Fatalf("put a2: %v", err)
	}
	if err := store.Delete([]byte("b")); err != nil {
		t.Fatalf("delete b: %v", err)
	}
	if err := store.Put([]byte("c"), []byte("4")); err != nil {
		t.Fatalf("put c: %v", err)
	}
	_ = waiter.Wait(t)

	snap := store.Snapshot()
	it := snap.Range(nil, nil)
	var keys [][]byte
	var vals [][]byte
	for it.Next() {
		entry := it.Entry()
		keys = append(keys, entry.Key)
		vals = append(vals, entry.Value)
	}
	if err := it.Err(); err != nil {
		if cerr := snap.Close(); cerr != nil {
			t.Errorf("close snapshot: %v", cerr)
		}
		t.Fatalf("range err: %v", err)
	}
	if err := snap.Close(); err != nil {
		t.Fatalf("close snapshot: %v", err)
	}

	if len(keys) != 2 {
		t.Fatalf("expected 2 keys after compaction, got %d", len(keys))
	}
	if !bytes.Equal(keys[0], []byte("a")) || !bytes.Equal(vals[0], []byte("3")) {
		t.Fatalf("expected a=3, got %q=%q", keys[0], vals[0])
	}
	if !bytes.Equal(keys[1], []byte("c")) || !bytes.Equal(vals[1], []byte("4")) {
		t.Fatalf("expected c=4, got %q=%q", keys[1], vals[1])
	}
	if got, ok := store.Get([]byte("b")); ok || !got.Tombstone {
		t.Fatalf("expected b deleted, ok=%v entry=%+v", ok, got)
	}

	if err := store.Close(); err != nil {
		t.Fatalf("close: %v", err)
	}

	reopened, err := lsm.New(lsm.Options{DataDir: dir})
	if err != nil {
		t.Fatalf("reopen: %v", err)
	}
	t.Cleanup(func() {
		if err := reopened.Close(); err != nil {
			t.Errorf("close reopened: %v", err)
		}
	})

	if got, ok := reopened.Get([]byte("a")); !ok || !bytes.Equal(got.Value, []byte("3")) {
		t.Fatalf("expected a=3 after reopen, ok=%v val=%q", ok, got.Value)
	}
	if got, ok := reopened.Get([]byte("b")); ok || !got.Tombstone {
		t.Fatalf("expected b deleted after reopen, ok=%v entry=%+v", ok, got)
	}
	if got, ok := reopened.Get([]byte("c")); !ok || !bytes.Equal(got.Value, []byte("4")) {
		t.Fatalf("expected c=4 after reopen, ok=%v val=%q", ok, got.Value)
	}
}
