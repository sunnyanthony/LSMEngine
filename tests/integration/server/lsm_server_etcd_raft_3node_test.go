...
func (t *asyncInProcessRaftTransport) Send(_ context.Context, messages []lsm.CommitLogPeerMessage) error {
	for _, message := range messages {
		...
		go func() {
			...
			defer cancel()
			if err := target.HandlePeerMessages(ctx, []lsm.CommitLogPeerMessage{msg}); err != nil {
				t.recordErr(err)
			}
		}()
	}
...
func TestEtcdRaftHTTPDynamicJoinReplicatesToNewNode(t *testing.T) {
	...
	eventually(t, 5*time.Second, func() bool {
		entry, ok := stores["node-c"].Get([]byte("seed"))
		return ok && string(entry.Value) == "before-join"
	})
