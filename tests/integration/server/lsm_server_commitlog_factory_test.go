...
func (integrationCommitLogConsensus) HandlePeerMessages(_ context.Context, _ []lsm.CommitLogPeerMessage) error {
	return nil
}
...
func (c *integrationPeerIngressConsensus) HandlePeerMessages(_ context.Context, messages []lsm.CommitLogPeerMessage) error {
	c.peerCalls.Add(1)
...
func TestServerCommitLogFactorySupportsPeerIngress(t *testing.T) {
	...
	if err := store.HandlePeerMessages(context.Background(), []lsm.CommitLogPeerMessage{
