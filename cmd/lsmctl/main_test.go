...
func TestSelectedGatewayRetryPolicyAllowsExplicitZeroFlags(t *testing.T) {
	cfg := serverconfig.Config{
		GatewayMaxWriteAttempts:  4,
		GatewayWriteRetryBackoff: 25 * time.Millisecond,
	}
	if got := selectedGatewayMaxWriteAttempts(cfg, 0, true); got != 0 {
		t.Fatalf("expected explicit zero max attempts flag, got %d", got)
	}
	if got := selectedGatewayWriteRetryBackoff(cfg, 0, true); got != 0 {
		t.Fatalf("expected explicit zero retry backoff flag, got %v", got)
	}
}

func TestGatewayDiscoverySelectedConfigRejectsConflictingSources(t *testing.T) {
	cfg := serverconfig.Config{
		GatewayEndpointFile:    "/unused/endpoints.yaml",
		GatewayEndpointDNSName: "cluster.example",
	}
	selected := selectedGatewayEndpointDiscoveryOptions(cfg, gatewayEndpointDiscoveryOptions{}, gatewayEndpointDiscoveryFlagState{})
	if _, err := gatewayNodeEndpointResolverFromConfig(cfg, "http://node-a", selected, nil); err == nil {
		t.Fatal("expected conflicting configured discovery sources to be rejected")
	}
}

func TestGatewayDiscoveryExplicitEmptySourceUsesStaticResolver(t *testing.T) {
	for _, flags := range []gatewayEndpointDiscoveryFlagState{{EndpointFile: true}, {DNSSRVName: true}} {
		cfg := serverconfig.Config{
			GatewayEndpointFile:    "/unused/endpoints.yaml",
			GatewayEndpointDNSName: "cluster.example",
		}
		selected := selectedGatewayEndpointDiscoveryOptions(cfg, gatewayEndpointDiscoveryOptions{}, flags)
		resolver, err := gatewayNodeEndpointResolverFromConfig(cfg, "http://node-a", selected, nil)
		if err != nil {
			t.Fatal(err)
		}
		endpoints, err := resolver.ResolveNodeEndpoints(context.Background())
		if err != nil || len(endpoints) == 0 {
			t.Fatalf("explicit empty source failed static fallback: %v %v", endpoints, err)
		}
	}
}
...
func TestSelectedGatewayEndpointDiscoveryDNSNameFlagOverridesConfigFile(t *testing.T) {
	got := selectedGatewayEndpointDiscoveryOptions(serverconfig.Config{
		GatewayEndpointFile: "/var/lib/lsm/config-endpoints.yaml",
	}, gatewayEndpointDiscoveryOptions{
		DNSSRVName: "cli.lsm-cluster.svc.cluster.local",
	...
	if got.EndpointFile != "" {
		t.Fatalf("expected DNS name flag to suppress config endpoint file, got %+v", got)
	}
	if got.DNSSRVName != "cli.lsm-cluster.svc.cluster.local" {
		t.Fatalf("expected flag dns name, got %+v", got)
	}
...
func TestSelectedGatewayEndpointDiscoveryFileFlagOverridesConfigDNSName(t *testing.T) {
	got := selectedGatewayEndpointDiscoveryOptions(serverconfig.Config{
		GatewayEndpointDNSName: "config.lsm-cluster.svc.cluster.local",
	}, gatewayEndpointDiscoveryOptions{
		EndpointFile: "/tmp/cli-endpoints.yaml",
	...
	if got.DNSSRVName != "" {
		t.Fatalf("expected endpoint file flag to suppress config dns name, got %+v", got)
	}
	if got.EndpointFile != "/tmp/cli-endpoints.yaml" {
		t.Fatalf("expected flag endpoint file, got %+v", got)
	}
...
func TestReadGatewayStatusDecodesUnavailableStatus(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/gateway/status" {
			t.Fatalf("unexpected path %s", r.URL.Path)
		}
		...
		_ = json.NewEncoder(w).Encode(server.GatewayClusterStatus{
			Ready:          false,
			Reason:         "no reachable node endpoints",
			NodeCount:      2,
			ReachableNodes: 0,
			Nodes: []server.GatewayClusterNodeStatus{
				{Node: "node-a", Endpoint: "http://node-a", Error: "connection refused"},
			},
		})
	}))
	defer srv.Close()
	...
	got, err := readGatewayStatus(srv.URL)
	if err != nil {
		t.Fatalf("read unavailable gateway status: %v", err)
	}
	if got.Ready || got.Reason == "" || got.NodeCount != 2 || len(got.Nodes) != 1 {
		t.Fatalf("unexpected gateway status: %+v", got)
	}
}

func TestReadGatewayStatusRejectsEmptyErrorPayload(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := readGatewayStatus(srv.URL); err == nil {
		t.Fatalf("expected empty error payload to fail")
	}
}
...
func TestWaitGatewayStatusDeadlineBoundsPolling(t *testing.T) {
	for _, slow := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				_ = json.NewEncoder(w).Encode(server.GatewayClusterStatus{NodeCount: 3})
			}))
			defer srv.Close()
			start := time.Now()
			got, err := waitGatewayStatus(srv.URL, waitGatewayOptions{
				Timeout: 50 * time.Millisecond, Interval: 3 * time.Second,
			})
			if got.Ready || !errors.Is(err, context.DeadlineExceeded) {
				t.Fatalf("expected deadline, got ready=%v err=%v", got.Ready, err)
			}
			if elapsed := time.Since(start); elapsed > time.Second {
				t.Fatalf("wait exceeded deadline by too much: %v", elapsed)
			}
		})
	}
}

func TestGatewayWaitReadGateRequiresSelectedLeader(t *testing.T) {
	limit := uint64(0)
	status := server.GatewayClusterStatus{Ready: true, NodeCount: 2, ReachableNodes: 2,
		ReadMode: "leader", WriteLeader: "leader", WriteLeaderEndpoint: "http://leader",
		Nodes: []server.GatewayClusterNodeStatus{
			{Node: "leader", Endpoint: "http://leader", OK: true, Status: &lsm.ClusterStatus{CommitLogRuntime: lsm.CommitLogRuntimeStatus{Health: "ready", ApplyLag: 4}}},
			{Node: "follower", Endpoint: "http://follower", OK: true, Status: &lsm.ClusterStatus{CommitLogRuntime: lsm.CommitLogRuntimeStatus{Health: "follower", ApplyLag: 0}}},
		},
	}
	opts := waitGatewayOptions{MaxReadApplyLag: &limit}
	if got := evaluateGatewayWait(status, opts); got.Ready || got.ReadReadyNodes != 0 {
		t.Fatalf("follower masked lagging leader: %+v", got)
	}
	status.Nodes[0].Status.CommitLogRuntime.ApplyLag = 0
	if got := evaluateGatewayWait(status, opts); !got.Ready || got.ReadReadyNodes != 1 {
		t.Fatalf("ready leader not accepted: %+v", got)
	}
	status.WriteLeaderEndpoint = ""
	if got := evaluateGatewayWait(status, opts); got.Ready {
		t.Fatalf("missing leader target accepted: %+v", got)
	}
}
...
func TestWriteGatewayStatus(t *testing.T) {
	...
	for _, want := range []string{
		...
		"routing_read_attempts=5",
		"routing_read_fallbacks=2",
		"routing_read_failures=1",
...
func TestWriteStatsIncludesWALStats(t *testing.T) {
	...
	writeStats(&buf, lsm.Stats{
		...
		WriteBackpressure: lsm.WriteBackpressureStats{
			WALCheckpointLag:      3,
			WALCheckpointLagLimit: 8,
		},
		WAL: lsm.WALStats{
			SegmentID:                    4,
			SegmentCount:                 3,
			...
			CheckpointSeq:                12,
			CheckpointLag:                3,
			ReadyMaxCheckpointLag:        10,
			BackpressureMaxCheckpointLag: 8,
			MaxSegmentBytes:              8192,
			RetainArchivedSegments:       2,
			BlockSize:                    65536,
	...
	for _, want := range []string{
		...
		"write_backpressure_wal_checkpoint_lag=3",
		"write_backpressure_wal_checkpoint_lag_limit=8",
		"wal_segment_id=4",
		"wal_segments=3",
		...
		"wal_checkpoint_seq=12",
		"wal_checkpoint_lag=3",
		"wal_ready_max_checkpoint_lag=10",
		"wal_backpressure_max_checkpoint_lag=8",
...
func TestApplyPlannedReplacementRetriesTransientApplyFailure(t *testing.T) {
	for _, lateFailure := range []bool{false, true} {
		t.Run(fmt.Sprintf("late=%v", lateFailure), func(t *testing.T) {
			var raftAddCalls atomic.Int32
			var raftRemoveCalls atomic.Int32
			var addReplicaCalls atomic.Int32
			var drainCalls atomic.Int32
			...
			nodeA := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				// The initially selected node recovers between attempts. Retrying must
				// retain the original plan rather than attempt candidate selection again.
				_ = json.NewEncoder(w).Encode(lsm.ClusterStatus{
					NodeID: "node-a", Draining: drainCalls.Load() > 0,
					CommitLogRuntime: lsm.CommitLogRuntimeStatus{Health: "follower", LeaderKnown: true},
				})
			}))
			nodeAURL := nodeA.URL
			...
			nodeB := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				case "/cluster/nodes/node-d/raft-add":
					if raftAddCalls.Add(1) == 1 && !lateFailure {
				...
				case "/cluster/shards/users/remove-replica":
					...
					_ = json.NewEncoder(w).Encode(map[string]string{"status": "ok"})
				case "/cluster/nodes/node-a/raft-remove":
					if raftRemoveCalls.Add(1) == 1 && lateFailure {
						http.Error(w, "temporary raft-remove failure", http.StatusServiceUnavailable)
						return
					}
					_ = json.NewEncoder(w).Encode(map[string]string{"status": "ok"})
				default:
			...
			if raftAddCalls.Load() != 2 || addReplicaCalls.Load() != expected || drainCalls.Load() != expected || removeReplicaCalls.Load() != expected || raftRemoveCalls.Load() != expected {
				t.Fatalf("unexpected call counts raftAdd=%d add=%d drain=%d remove=%d raftRemove=%d",
					raftAddCalls.Load(), addReplicaCalls.Load(), drainCalls.Load(), removeReplicaCalls.Load(), raftRemoveCalls.Load())
			}
		})
...
func TestToCommitLogOptionsBuildsTransportWithPeerURLFile(t *testing.T) {
	...
	if err := opts.Transport.Send(context.Background(), []lsm.CommitLogPeerMessage{
...
func TestParseBytesFlagFromFlagSetAllowsExplicitEmptyValue(t *testing.T) {
	fs := flag.NewFlagSet("put", flag.ContinueOnError)
	value := fs.String("value", "", "")
	valueBase64 := fs.String("value-base64", "", "")
	if err := fs.Parse([]string{"--value", ""}); err != nil {
		t.Fatalf("parse: %v", err)
	}
	got, err := parseBytesFlagFromFlagSet(fs, "value", *value, *valueBase64)
	if err != nil {
		t.Fatalf("parse explicit empty value: %v", err)
	}
	if len(got) != 0 {
...
func TestParseBytesFlagFromFlagSetAllowsExplicitEmptyBase64Value(t *testing.T) {
	fs := flag.NewFlagSet("put", flag.ContinueOnError)
	value := fs.String("value", "", "")
	valueBase64 := fs.String("value-base64", "", "")
	if err := fs.Parse([]string{"--value-base64", ""}); err != nil {
		t.Fatalf("parse: %v", err)
	}
	got, err := parseBytesFlagFromFlagSet(fs, "value", *value, *valueBase64)
	if err != nil {
		t.Fatalf("parse explicit empty base64 value: %v", err)
	}
	if len(got) != 0 {
		t.Fatalf("expected empty value, got %q", got)
	}
...
func TestReadKVRemoteNotFound(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(kvGetResult{
			Found:     false,
			KeyBase64: base64.StdEncoding.EncodeToString([]byte("missing")),
		})
	}))
	defer server.Close()
...
func TestReadKVRemoteEndpointNotFoundReturnsError(t *testing.T) {
	server := httptest.NewServer(http.NotFoundHandler())
	defer server.Close()

	_, err := readKV(server.URL, "", []byte("missing"))
	if err == nil {
		t.Fatalf("expected endpoint 404 to return error")
	}
}
...
func TestReadKVRangeRejectsInvalidLimit(t *testing.T) {
	...
	if _, err := readKVRange("http://example.test", "", nil, nil, maxKVRangeLimit+1); err == nil {
		t.Fatalf("expected max limit error")
	}
}
