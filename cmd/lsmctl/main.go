...
func serveCmd(args []string) {
	...
	store, err := lsm.New(lsm.Options{
		...
		CompactionAdaptiveCheck:           cfg.CompactionAdaptiveCheck,
...
func gatewayCmd(args []string) {
	...
	gatewayReadBalancePolicy := selectedGatewayReadBalancePolicy(cfg, *readBalancePolicy)
...
func selectedGatewayReadBalancePolicy(cfg serverconfig.Config, flagValue string) string {
	if flagValue != "" {
		return flagValue
	}
	return cfg.GatewayReadBalancePolicy
}
...
func writeStats(w io.Writer, stats lsm.Stats) {
	...
	fmt.Fprintf(w, "immutables=%d\n", stats.ImmutableCount)
	fmt.Fprintf(w, "immutable_bytes=%d\n", stats.ImmutableBytes)
	fmt.Fprintf(w, "flush_queue_depth=%d\n", stats.FlushQueueDepth)
	fmt.Fprintf(w, "flush_queue_capacity=%d\n", stats.FlushQueueCapacity)
	fmt.Fprintf(w, "pinned=%d\n", stats.PinnedCount)
	fmt.Fprintf(w, "tables=%d\n", stats.TableCount)
	fmt.Fprintf(w, "sstables=%d\n", stats.SSTableCount)
	fmt.Fprintf(w, "sstable_bytes=%d\n", stats.SSTableBytes)
	...
	fmt.Fprintf(w, "compaction_adaptive_check=%v\n", stats.CompactionAdaptiveCheck)
	fmt.Fprintf(w, "compaction_effective_check_interval_ms=%d\n", stats.CompactionEffectiveCheckIntervalMS)
	...
	fmt.Fprintf(w, "point_reads=%d\n", stats.PointReads)
	fmt.Fprintf(w, "point_read_memtable_hits=%d\n", stats.PointReadMemtableHits)
	fmt.Fprintf(w, "point_read_immutable_hits=%d\n", stats.PointReadImmutableHits)
	fmt.Fprintf(w, "point_read_sstable_hits=%d\n", stats.PointReadSSTableHits)
	fmt.Fprintf(w, "point_read_misses=%d\n", stats.PointReadMisses)
	fmt.Fprintf(w, "point_read_sstable_probes=%d\n", stats.PointReadSSTableProbes)
	fmt.Fprintf(w, "point_read_max_sstable_probes=%d\n", stats.PointReadMaxSSTableProbes)
	fmt.Fprintf(w, "sstable_flow_cache_hit=%d\n", stats.SSTableFlow.CacheHit)
	fmt.Fprintf(w, "sstable_flow_cache_miss=%d\n", stats.SSTableFlow.CacheMiss)
	fmt.Fprintf(w, "sstable_flow_filter_pass=%d\n", stats.SSTableFlow.FilterPass)
	fmt.Fprintf(w, "sstable_flow_filter_skip=%d\n", stats.SSTableFlow.FilterSkip)
	fmt.Fprintf(w, "sstable_flow_errors=%d\n", stats.SSTableFlow.Errors)
	fmt.Fprintf(w, "compaction_triggers=%d\n", stats.CompactionRuntime.Triggers)
	fmt.Fprintf(w, "compaction_coalesced_triggers=%d\n", stats.CompactionRuntime.CoalescedTriggers)
	fmt.Fprintf(w, "compaction_runs=%d\n", stats.CompactionRuntime.Runs)
	fmt.Fprintf(w, "compaction_steps=%d\n", stats.CompactionRuntime.Steps)
	fmt.Fprintf(w, "compaction_successful_steps=%d\n", stats.CompactionRuntime.SuccessfulSteps)
	fmt.Fprintf(w, "compaction_errors=%d\n", stats.CompactionRuntime.Errors)
	fmt.Fprintf(w, "compaction_running=%v\n", stats.CompactionRuntime.Running)
	fmt.Fprintf(w, "write_backpressure_active=%v\n", stats.WriteBackpressure.Active)
	fmt.Fprintf(w, "write_backpressure_reason=%s\n", stats.WriteBackpressure.Reason)
	fmt.Fprintf(w, "write_backpressure_rejects=%d\n", stats.WriteBackpressure.Rejects)
	...
	fmt.Fprintf(w, "wal_active_segment_bytes=%d\n", stats.WAL.ActiveSegmentBytes)
	...
	fmt.Fprintf(w, "wal_pending_block_bytes=%d\n", stats.WAL.PendingBlockBytes)
	fmt.Fprintf(w, "wal_pending_block_records=%d\n", stats.WAL.PendingBlockRecords)
	fmt.Fprintf(w, "wal_sync=%v\n", stats.WAL.Sync)
	fmt.Fprintf(w, "wal_async=%v\n", stats.WAL.Async)
	fmt.Fprintf(w, "wal_closed=%v\n", stats.WAL.Closed)
	if stats.WAL.SegmentScanError != "" {
		fmt.Fprintf(w, "wal_segment_scan_error=%q\n", stats.WAL.SegmentScanError)
	}
	fmt.Fprintf(w, "seq=%d\n", stats.Seq)
	fmt.Fprintf(w, "closing=%v closed=%v\n", stats.Closing, stats.Closed)
	fmt.Fprintf(w, "flush_blocked=%v compaction_enabled=%v\n", stats.FlushBlocked, stats.CompactionEnabled)
}
...
const defaultKVRangeLimit = 100
const maxKVRangeLimit = 1000
...
func getCmd(args []string) {
	...
	keyBytes, err := parseBytesFlagFromFlagSet(fs, "key", *key, *keyBase64)
	if err != nil {
		log.Fatal(err)
	}
...
func rangeCmd(args []string) {
	...
	limit := fs.Int("limit", defaultKVRangeLimit, "maximum entries to return")
	jsonOut := fs.Bool("json", false, "emit JSON")
	if err := fs.Parse(args); err != nil {
		log.Fatal(err)
	}
...
func putCmd(args []string) {
	...
	valueBytes, err := parseBytesFlagFromFlagSet(fs, "value", *value, *valueBase64)
	if err != nil {
		log.Fatal(err)
	}
...
func asyncPutCmd(args []string) {
	...
	if *addr == "" {
		log.Fatal("async-put requires --addr or config addr")
	}
	keyBytes, err := parseBytesFlagFromFlagSet(fs, "key", *key, *keyBase64)
	if err != nil {
		log.Fatal(err)
	...
	status, err := writeKVPut(*addr, "", keyBytes, valueBytes, lsm.WriteConsistencyAccepted)
	if err != nil {
		log.Fatal(err)
	}
	writeKVStatus(os.Stdout, status, *jsonOut)
...
func asyncDeleteCmd(args []string) {
	...
	if *addr == "" {
		log.Fatal("async-delete requires --addr or config addr")
	}
	keyBytes, err := parseBytesFlagFromFlagSet(fs, "key", *key, *keyBase64)
	if err != nil {
		log.Fatal(err)
	}
	status, err := writeKVDelete(*addr, "", keyBytes, lsm.WriteConsistencyAccepted)
	if err != nil {
		log.Fatal(err)
	}
	writeKVStatus(os.Stdout, status, *jsonOut)
	...
	if addr != "" {
		...
		if err != nil {
			...
			if errors.As(err, &statusErr) && statusErr.StatusCode == http.StatusNotFound {
				var missing kvGetResult
				if json.Unmarshal([]byte(statusErr.Body), &missing) == nil && !missing.Found {
					if missing.KeyBase64 == "" {
						missing.KeyBase64 = base64.StdEncoding.EncodeToString(key)
					}
					return missing, nil
				}
			}
...
func readKVRange(addr, dataDir string, start []byte, end []byte, limit int) (kvRangeResult, error) {
	...
	if limit > maxKVRangeLimit {
		return kvRangeResult{}, fmt.Errorf("limit must be <= %d", maxKVRangeLimit)
	}
	if addr != "" {
		query := url.Values{}
		if start != nil {
...
func readGatewayStatus(addr string) (server.GatewayClusterStatus, error) {
	return readGatewayStatusContext(context.Background(), addr)
}

func readGatewayStatusContext(ctx context.Context, addr string) (server.GatewayClusterStatus, error) {
	if strings.TrimSpace(addr) == "" {
		return server.GatewayClusterStatus{}, fmt.Errorf("gateway-status requires --addr or config addr")
	}
	client := http.Client{Timeout: 2 * time.Second}
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, normalizeHTTPBaseURL(addr)+"/gateway/status", nil)
	if err != nil {
	...
	if resp.StatusCode >= 400 {
		...
		if err := json.Unmarshal(body, &status); err == nil && gatewayStatusPayloadPresent(status) {
			return status, nil
		}
		return server.GatewayClusterStatus{}, &httpStatusError{
...
func gatewayStatusPayloadPresent(status server.GatewayClusterStatus) bool {
	return status.Ready ||
		status.Reason != "" ||
		status.NodeCount != 0 ||
		status.ReachableNodes != 0 ||
		status.WriteLeader != "" ||
...
func waitGatewayStatus(addr string, opts waitGatewayOptions) (gatewayWaitResult, error) {
	...
	var lastErr error
	for {
		status, err := readGatewayStatusContext(ctx, addr)
		if err != nil {
			lastErr = err
		} else {
			last = evaluateGatewayWait(status, opts)
			if last.Ready && ctx.Err() == nil {
				return last, nil
			}
		}
		if ctx.Err() != nil {
			...
			return last, fmt.Errorf("%s: %w", message, ctx.Err())
		}
...
func evaluateGatewayWait(status server.GatewayClusterStatus, opts waitGatewayOptions) gatewayWaitResult {
	...
	if opts.MaxReadApplyLag != nil {
		...
		readNodes := status.Nodes
		if status.ReadMode == string(server.GatewayReadModeLeader) {
			readNodes = nil
			for _, node := range status.Nodes {
				if status.WriteLeaderEndpoint != "" && node.Endpoint == status.WriteLeaderEndpoint {
					readNodes = append(readNodes, node)
					break
				}
			}
		}
		result.ReadReadyNodes = gatewayReadReadyNodes(readNodes, *opts.MaxReadApplyLag)
...
func evaluateClusterWait(statuses clusterStatusResult, opts waitClusterOptions) clusterWaitResult {
	...
	for _, node := range statuses.Nodes {
		...
		if (opts.MaxApplyLag != nil || opts.MinAppliedIndex != nil) && !nodeReady {
			continue
		}
...
func compatibilityStatusKnown(status lsm.CompatibilityStatus) bool {
	return status.ClusterStatusVersion > 0 &&
		status.ControlStateVersion > 0 &&
		status.StateSnapshotVersion > 0 &&
		status.RaftPeerMessageVersion > 0
}
...
func parseBytesFlagFromFlagSet(fs *flag.FlagSet, name string, text string, encoded string) ([]byte, error) {
	textSet := flagWasSet(fs, name)
	encodedSet := flagWasSet(fs, name+"-base64")
	if textSet && encodedSet {
