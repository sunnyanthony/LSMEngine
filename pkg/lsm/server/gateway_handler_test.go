...
func TestGatewayHandlerGetOrderedReadBalanceUsesFirstHealthyEndpoint(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		rec := httptest.NewRecorder()
		handler.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/kv/get?key_base64=Yw==", nil))
		if rec.Code != http.StatusOK {
			t.Fatalf("fallback %d failed: %d", i, rec.Code)
		}
	}
	if nodeAReads.Load() != 4 || nodeBReads.Load() != 2 {
		t.Fatalf("ordered policy did not defer failed endpoint: a=%d b=%d", nodeAReads.Load(), nodeBReads.Load())
	}
}
...
func TestGatewayLeaderReadsDoNotFallBackAfterDiscovery(t *testing.T) {
	for _, path := range []string{"/kv/get", "/kv/range"} {
		for _, status := range []int{http.StatusNotFound, http.StatusServiceUnavailable} {
			t.Run(fmt.Sprintf("%s/%d", path, status), func(t *testing.T) {
				var followerReads, leaderReads int
				handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					leader := r.URL.Host == "node-b"
					if r.URL.Path == "/cluster/status" {
						writeJSON(w, http.StatusOK, lsm.ClusterStatus{CommitLogRuntime: lsm.CommitLogRuntimeStatus{
							Leader: leader, WriteAvailable: leader,
						}})
						return
					}
					if leader {
						leaderReads++
						writeJSON(w, status, getResponse{Found: false})
					} else {
						followerReads++
						writeJSON(w, http.StatusOK, getResponse{Found: true, Seq: 1})
					}
				...
				gateway, err := NewGateway(GatewayOptions{
					BootstrapURL: "http://node-a", ReadMode: GatewayReadModeLeader,
					NodeEndpoints: map[string]string{"node-a": "http://node-a", "node-b": "http://node-b"},
					HTTPClient:    newInMemoryHTTPClient(map[string]http.Handler{"node-a": handler, "node-b": handler}),
				})
				if err != nil {
					t.Fatal(err)
				}
				rec := httptest.NewRecorder()
				NewGatewayHandler(gateway, HandlerOptions{}).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, path, nil))
				if rec.Code != status || followerReads != 0 || leaderReads != 1 {
					t.Fatalf("unexpected fallback: status=%d follower=%d leader=%d", rec.Code, followerReads, leaderReads)
				}
			})
		}
	}
