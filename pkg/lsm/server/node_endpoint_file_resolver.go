...
func parseNodeEndpointFile(data []byte) (map[string]string, error) {
	...
	endpoints, err := normalizeNodeEndpointMapStrict(raw)
	if err != nil {
		return nil, fmt.Errorf("parse node endpoint file: %w", err)
	}
	if len(endpoints) == 0 {
...
func normalizeNodeEndpointMapStrict(in map[string]string) (map[string]string, error) {
	...
	for nodeID, endpoint := range in {
		...
		out[nodeID] = NormalizeHTTPBaseURL(endpoint)
	}
	return out, nil
}
