...
func TestValidateEtcdRaftAllowsSinglePeerWithoutPeerURLs(t *testing.T) {
	cfg := Config{
		NodeID: "node-a",
		CommitLog: CommitLogConfig{
			Provider: "etcd-raft",
	...
	if err := Validate(cfg); err != nil {
		t.Fatalf("validate single peer: %v", err)
	}
}

func TestValidateEtcdRaftJoinRequiresExistingPeer(t *testing.T) {
	cfg := Config{
		NodeID: "node-a",
		CommitLog: CommitLogConfig{
			Provider: "etcd-raft",
	...
	if err := Validate(cfg); err == nil {
		t.Fatalf("expected join peer requirement validation error")
	}
}

func TestValidateEtcdRaftRejectsRelativePeerURL(t *testing.T) {
	cfg := Config{
		NodeID: "node-a",
		CommitLog: CommitLogConfig{
			Provider: "etcd-raft",
	...
	if err := Validate(cfg); err == nil {
		t.Fatalf("expected relative peer url validation error")
	}
}
