...
func NewNodeEndpointDNSResolver(opts NodeEndpointDNSResolverOptions) (*NodeEndpointDNSResolver, error) {
	...
	if scheme != "http" && scheme != "https" {
		return nil, fmt.Errorf("node endpoint DNS scheme must be http or https")
	}
	lookup := opts.LookupSRV
	if lookup == nil {
		lookup = net.DefaultResolver.LookupSRV
	}
	fallback, err := normalizeNodeEndpointMapStrict(opts.FallbackNodeEndpoints)
	if err != nil {
		return nil, err
	}
...
func nodeEndpointsFromSRVRecords(scheme string, records []*net.SRV) (map[string]string, error) {
	...
	for _, record := range records {
		...
		if target == "" || nodeID == "" || record.Port == 0 {
			return nil, fmt.Errorf("node endpoint SRV lookup returned invalid endpoint")
		}
		endpoint := NormalizeHTTPBaseURL(fmt.Sprintf("%s://%s:%d", scheme, target, record.Port))
		if _, exists := endpoints[nodeID]; exists {
			return nil, fmt.Errorf("node endpoint SRV lookup returned duplicate node %q", nodeID)
		}
