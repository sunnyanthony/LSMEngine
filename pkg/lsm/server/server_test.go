...
func (stubProvider) Stats() lsm.Stats {
	return lsm.Stats{
		...
		L0TableCount:                       2,
		L0SizeBytes:                        40,
		CompactionL0Threshold:              4,
		CompactionCheckIntervalMS:          30000,
		CompactionAdaptiveCheck:            true,
		CompactionEffectiveCheckIntervalMS: 7500,
		...
		WriteBackpressure: lsm.WriteBackpressureStats{
			Active:                 true,
			Reason:                 "l0_compaction_pressure",
			Rejects:                2,
			FlushQueueThreshold:    3,
			CompactionL0Threshold:  5,
			CompactionL0TableCount: 2,
			WALCheckpointLag:       9,
			WALCheckpointLagLimit:  6,
		},
		WAL: lsm.WALStats{
			SegmentCount:                 3,
			...
			TotalBytes:                   512,
			CheckpointLag:                9,
			ReadyMaxCheckpointLag:        20,
			BackpressureMaxCheckpointLag: 6,
			MaxSegmentBytes:              1024,
			RetainArchivedSegments:       7,
			BlockSize:                    32768,
...
func (p *raftStubProvider) HandlePeerMessages(_ context.Context, messages []lsm.CommitLogPeerMessage) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *raftStubProvider) messagesCopy() []lsm.CommitLogPeerMessage {
	p.mu.Lock()
	defer p.mu.Unlock()
	out := make([]lsm.CommitLogPeerMessage, len(p.messages))
	copy(out, p.messages)
	return out
}
...
func TestServerMetricsReportsPartialWALScan(t *testing.T) {
	for _, scanError := range []string{"", "permission denied"} {
		var out strings.Builder
		stats := stubProvider{}.Stats()
		stats.WAL.SegmentScanError = scanError
		writeServerMetrics(&out, stats)
		want := "lsm_engine_wal_segment_scan_error 0\n"
		if scanError != "" {
			want = "lsm_engine_wal_segment_scan_error 1\n"
		}
		if !strings.Contains(out.String(), want) || !strings.Contains(out.String(), "lsm_engine_wal_bytes 512\n") {
			t.Fatalf("partial scan signal or measured bytes missing: %s", out.String())
		}
	}
}
...
func TestHandlerMetrics(t *testing.T) {
	...
	for _, want := range []string{
		...
		"lsm_engine_compaction_l0_threshold 4",
		"lsm_engine_compaction_check_interval_seconds 30",
		"lsm_engine_compaction_adaptive_check 1",
		"lsm_engine_compaction_effective_check_interval_seconds 7.5",
		...
		"lsm_engine_write_backpressure_flush_queue_threshold 3",
		"lsm_engine_write_backpressure_compaction_l0_threshold 5",
		"lsm_engine_write_backpressure_compaction_l0_tables 2",
		"lsm_engine_write_backpressure_wal_checkpoint_lag 9",
		"lsm_engine_write_backpressure_wal_checkpoint_lag_limit 6",
		...
		"lsm_engine_wal_ready_max_checkpoint_lag 20",
		"lsm_engine_wal_backpressure_max_checkpoint_lag 6",
		"lsm_engine_wal_max_segment_bytes 1024",
		"lsm_engine_wal_retain_archived_segments 7",
		"lsm_engine_wal_block_size 32768",
	} {
...
func TestHandlerMetricsUnavailableWithoutProvider(t *testing.T) {
	...
	out := rec.Body.String()
	for _, want := range []string{
		"lsm_engine_memtable_bytes 0",
		"lsm_engine_wal_segments 0",
		"lsm_engine_closed 0",
	} {
		if !strings.Contains(out, want) {
			t.Fatalf("expected metrics to contain %q, got:\n%s", want, out)
...
func TestHandlerRaftPeerMessages(t *testing.T) {
	...
	handler := NewHandler(provider)
	body := `{"messages":[{"from":1,"to":2,"payload":"AQID"}]}`
	...
	if messages[0].From != 1 || messages[0].To != 2 {
