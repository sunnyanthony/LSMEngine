...
func decodeStateSnapshotForRaftIndex(index uint64, data []byte) (lsmStateSnapshot, error) {
	...
	if err := validateLSMStateSnapshot(snapshot); err != nil {
		return lsmStateSnapshot{}, err
	}
...
func validateLSMStateSnapshot(snapshot lsmStateSnapshot) error {
	for _, entry := range snapshot.Entries {
		...
		if entry.Seq > snapshot.Seq {
			return fmt.Errorf("state snapshot entry sequence exceeds snapshot sequence")
		}
	}
	return nil
}
