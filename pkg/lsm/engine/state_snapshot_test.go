...
func TestStateSnapshotRestoreRejectsMalformedEntriesBeforeWriting(t *testing.T) {
	tests := []struct {
		name     string
	...
	}{
		{
			...
			snapshot: lsmStateSnapshot{
				...
				Seq:     1,
				Entries: []types.Entry{{Key: []byte("a"), Value: []byte("1")}},
			},
		},
		{
			name: "entry seq exceeds header",
			snapshot: lsmStateSnapshot{
				Version: lsmStateSnapshotVersion,
				Seq:     1,
				Entries: []types.Entry{{Key: []byte("a"), Value: []byte("1"), Seq: 2}},
			},
		},
		{
			name: "tombstone",
			snapshot: lsmStateSnapshot{
				Version: lsmStateSnapshotVersion,
				Seq:     2,
				Entries: []types.Entry{{Key: []byte("a"), Tombstone: true, Seq: 2}},
			},
		},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			payload, err := json.Marshal(tc.snapshot)
			if err != nil {
			...
			if _, ok := target.Get([]byte("a")); ok {
				t.Fatalf("malformed snapshot wrote data")
			}
			if target.seq != 0 {
				t.Fatalf("malformed snapshot advanced seq to %d", target.seq)
			}
		})
	}
}
