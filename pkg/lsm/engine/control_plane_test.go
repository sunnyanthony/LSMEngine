...
func TestControlPlaneRemoveLeaderReplicaRejectedWithEmptyReplicaList(t *testing.T) {
	store, err := New(Options{
		DataDir:   t.TempDir(),
		NodeID:    "node-a",
		ClusterID: "cluster-dev",
	...
	shard := store.control.shards["users"]
	shard.Replicas = nil
	store.control.shards["users"] = shard
	store.control.mu.Unlock()

	err = store.RemoveReplica("users", "node-a")
	if err == nil || !strings.Contains(err.Error(), "cannot remove leader replica") {
		t.Fatalf("expected leader removal error, got %v", err)
	}
	if got := store.ClusterStatus().Revision; got != 0 {
		t.Fatalf("expected revision to remain 0, got %d", got)
	}
...
func TestControlPlaneRejectsFutureStateVersion(t *testing.T) {
	...
	if data, err := os.ReadFile(statePath); err != nil || string(data) != payload {
		t.Fatalf("rejected future state was modified: %q, %v", data, err)
	}
}

func TestControlPlaneRejectsNegativeStateVersion(t *testing.T) {
	dataDir := t.TempDir()
	statePath := filepath.Join(dataDir, "control_state.json")
	payload := `{"version":-1,"node_id":"node-a","cluster_id":"cluster-dev","order":["users"],"shards":[{"id":"users","leader":"node-a","replicas":[{"node_id":"node-a","role":"leader","healthy":true}]}]}`
	if err := os.WriteFile(statePath, []byte(payload), 0o644); err != nil {
		t.Fatalf("write state: %v", err)
	}
	_, err := New(Options{
	...
	if !strings.Contains(err.Error(), "invalid control state version -1") {
		t.Fatalf("expected invalid version error, got %v", err)
	}
	if data, err := os.ReadFile(statePath); err != nil || string(data) != payload {
		t.Fatalf("rejected invalid state was modified: %q, %v", data, err)
	}
}
